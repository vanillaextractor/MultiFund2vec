...
#   Risky_Proportion — portfolio-level risk metric (redundant to 1-Risk_Free)
# Risk_Free is kept as it represents a real allocation fraction.
NOISE_COLS = ["Num_Assets", "Risky_Proportion"]
raw = raw[~raw["asset"].isin(NOISE_COLS)].copy()
print(f"  Removed noise columns: {NOISE_COLS}, kept Risk_Free")
...
# ── Ground-truth label derivation using actual 'label' column ─
NOISE_LABELS = {'Num_Assets', 'Risky_Proportion', 'Risk_Free'}

# Build a fund -> label -> total weight mapping
label_weight_map = {}
